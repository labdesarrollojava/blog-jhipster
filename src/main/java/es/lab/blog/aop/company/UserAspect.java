...
import es.lab.blog.domain.CompanyParameter;
...
private final Logger log = LoggerFactory.getLogger(UserAspect.class);
     ...
     */
    @Before(value = "execution(* es.lab.blog.service.UserService.createUser(..)) && args(userDTO, ..)")
     ...
     */
    @Before(value = "execution(*  es.lab.blog.service.UserService.updateUser(..)) && args(userDTO, ..)")
     ...
     * Adds tenant information to DTO.
     */
    @Before(value = "execution(* es.lab.blog.repository.UserRepository.save(..)) && args(user, ..)")
     ...
     */
    @Before(value = "execution(* es.lab.blog.service.UserService.getUserWithAuthoritiesByLogin(..)) && args(login, ..)")
     ...
     * Adds filtering to prevent display of information from another tenant
     */
    @Before(value = "execution(* es.lab.blog.service.UserService.getUserWithAuthorities(..)) && args(id, ..)")
